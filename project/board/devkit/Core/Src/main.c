...
int main(void) {
    ...
    MX_USART2_UART_Init();
    /* USER CODE BEGIN 2 */
    /* Unbuffered stdout so printf appears immediately (newlib + UART
     * retarget). */
    setvbuf(stdout, NULL, _IONBF, 0);

    {
        static const uint8_t k_boot[] =
            "\r\n[boot] USART2 115200 — if you see this, TX pin is "
            "OK\r\n";
        (void)HAL_UART_Transmit(
            &huart2, k_boot, sizeof(k_boot) - 1U, HAL_MAX_DELAY);
    }
    LOG("printf from main AAAA(before scheduler)\r\n");
    /* USER CODE END 2 */
    ...
    if (defaultTaskHandle == NULL) {
        static const uint8_t k_err[] =
            "\r\n[ERR] defaultTask not created (heap full or "
            "osThreadNew "
            "failed)\r\n";
        (void)HAL_UART_Transmit(
            &huart2, k_err, sizeof(k_err) - 1U, HAL_MAX_DELAY);
        Error_Handler();
    }

    {
        ...
        const int n =
            snprintf(buf,
                     sizeof(buf),
                     "\r\n[heap] free before app_init: %lu\r\n",
                     (unsigned long)heap_free);
        if (n > 0 && n < (int)sizeof(buf)) {
            (void)HAL_UART_Transmit(
                &huart2, (uint8_t *)buf, (uint16_t)n, HAL_MAX_DELAY);
        }
    }

    /* Application (C++): init before scheduler — see
     * app/devkit/app/app.cpp */
    app_init();
    ...
    {
        static const uint8_t k_fatal[] =
            "\r\n[FATAL] osKernelStart returned — scheduler did not "
            "run\r\n";
        (void)HAL_UART_Transmit(
            &huart2, k_fatal, sizeof(k_fatal) - 1U, HAL_MAX_DELAY);
    }
