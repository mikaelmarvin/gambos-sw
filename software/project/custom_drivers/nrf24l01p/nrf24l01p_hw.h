/**
 * @file nrf24l01p_hw.h
 * @brief nRF24L01+ register map, bit masks (datasheet §9), and SPI command
 * opcodes (Table 20 / §8.3.1).
 *
 * Note: R/W register commands embed a 5-bit register address in the low bits —
 * use NRF24_CMD_R_REG / NRF24_CMD_W_REG. Table 20 does not define a separate
 * ACTIVATE (0x50) SPI command; FEATURE-related behavior is configured via the
 * FEATURE register per the product specification.
 */
...
#define NRF24_RF_SETUP_RF_DR (1U << 3)
 ...
 * @param pipe 0–5
 */
#define NRF24_CMD_W_ACK_PAYLOAD(pipe)                                          \
    ((uint8_t)(0xA8U | ((uint8_t)(pipe)&0x07U)))
